...
import { getQuotes, getQuoteById, saveQuote, saveQuotesRaw, deleteQuote, getProducts, getSettings, getCurrentUserProfile, getSupabase, uploadFileToStorage, getSubscriptionLevel, getCustomerById, sendQuoteEmail, getQuoteEmailLogs, saveQuoteEmailLog } from './db.js?v=3.0.53';
import { formatCurrency, formatDate, showToast, formatDateTime, fileToBase64, compressImage, parseCombinedAddress, parseCompanyAddress, calculateItemTotals, calculateQuoteTotals } from './utils.js?v=3.0.53';
import { navigateToView, editQuote, duplicateQuoteAsTemplate, openLightbox } from './app.js?v=3.0.53';
import { isOffline, enqueueOfflinePhoto, checkOfflineAction, getOfflineQuotes } from './offline-cache.js?v=3.0.53';
  ...
  if (detailPhotoSave) {
    detailPhotoSave.addEventListener('click', async () => {
      ...
      if (isOffline()) {
        ...
        if (detailPhotoAddFields) detailPhotoAddFields.style.display = 'none';
        if (detailGalleryUpload) detailGalleryUpload.value = '';
        if (detailCameraUpload) detailCameraUpload.value = '';
