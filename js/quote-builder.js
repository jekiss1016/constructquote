...
import { getProducts, getSettings, saveQuote, checkJobIdUnique, saveSettings, getCustomers, getSupabase, getCurrentUserProfile, uploadFileToStorage } from './db.js?v=3.0.59';
import { formatCurrency, showToast, fileToBase64, generateJobIdSuggestion, compressImage, parseCombinedAddress, calculateItemTotals, calculateQuoteTotals } from './utils.js?v=3.0.59';
import { navigateToView, viewQuoteDetails, getPreviousViewId, openLightbox } from './app.js?v=3.0.59';
import { renderQuoteDetails } from './quotes-list.js?v=3.0.59';
import { openCustomerModalInline } from './customers.js?v=3.0.59';
import { isOffline, enqueueOfflinePhoto } from './offline-cache.js?v=3.0.59';
...
function setupBuilderListeners() {
  ...
  const cameraUpload = document.getElementById('builder-camera-upload');

  const handleBuilderPhotoSelect = async (e) => {
  ...
  if (galleryUpload) galleryUpload.addEventListener('change', handleBuilderPhotoSelect);
  if (cameraUpload) cameraUpload.addEventListener('change', handleBuilderPhotoSelect);
